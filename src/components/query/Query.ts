...
export class QueryBuilder {
  ...
  #addEntity(entity: any): void {
    ...
    if (!this.#entityTypes.includes(_type)) {
      this.#entityTypes.push(_type);
    }

    // instantiate profiles
    if (_type === IM.PROFILE) {
      this.#profiles.set(entity["@id"], new Profile(entity));
    }
  }
...
export class Profile extends Entity {
  ...
  constructor(entity: any) {
    ...
    //parse definition
    if (entity["http://endhealth.info/im#definition"] && isJsonString(entity["http://endhealth.info/im#definition"])) {
  ...
  #convertToDefinitionTree(definition: any): void {
    ...
    const _queue = cloneDeep(_definitionTree); //adds top level operator clauses to the queue
    while (_queue.length > 0) {
      ...
      if (_children && _currentItem["currentPath"] == "") {
        // root path
        set(_definitionTree, "children", _children);
      } else if (_children) {
        // all other paths
        set(_definitionTree, _currentItem["currentPath"] + ".children", _children);
      }
